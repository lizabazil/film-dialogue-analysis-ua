...
class SrtTranscriptParser:
    ...
    def parse(self, srt_file_path: str) -> list[EvalSegment]:
        ...
        for line in lines:
            if self._is_replica_number(line):  # exsessive info (id of replica), go to the next line
                if current_segment is not None and current_segment.speech.strip():
                    parsed_segments.append(current_segment)

                #current_segment = Segment()
                current_segment = EvalSegment()
                continue
            ...
            if self._is_timecode_string(line):  # have found the line with timecodes
                ...
                if line.startswith("-") and speaker and current_segment.speech and current_segment.speech.strip():
        ...
        if current_segment is not None and current_segment.speech and current_segment.speech.strip():
            parsed_segments.append(current_segment)
    ...
    def _get_speech(self, line: str) -> str:
        ...
        # clean from symbol '♪' which might be present in the subtitles
